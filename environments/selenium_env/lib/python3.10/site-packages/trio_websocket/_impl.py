    ...
    *,
    ...
    if ssl_context is None:
        ...
        stream = await trio.open_ssl_over_tcp_stream(
            host, port, ssl_context=ssl_context, https_compatible=True
        )
    if port in (80, 443):
        host_header = host
    else:
...
class CloseReason:
    ...
    def __repr__(self):
        ...
        return "{}<code={}, name={}, reason={}>".format(
            self.__class__.__name__, self.code, self.name, self.reason
        )
...
class WebSocketConnection(trio.abc.AsyncResource):
    ...
    async def _accept(self, request, subprotocol, extra_headers):
        ...
        await self._send(
            AcceptConnection(subprotocol=self._subprotocol, extra_headers=extra_headers)
        )
    ...
    async def _reject(self, status_code, headers, body):
        ...
        reject_conn = RejectConnection(
            status_code=status_code, headers=headers, has_body=bool(body)
        )
    ...
    async def _get_request(self):
        ...
        if self._connection_proposal is None:
            raise Exception(
                "No proposal available. Did you call this method"
    ...
    async def _handle_message_event(self, event):
        ...
        if self._message_size > self._max_message_size:
            ...
            msg = (b"" if isinstance(event, BytesMessage) else "").join(
                self._message_parts
            )
            self._message_size = 0
            self._message_parts = []
...
class Endpoint:
    ...
    def __repr__(self):
        ...
        return 'Endpoint(address="{}", port={}, is_ssl={})'.format(
            self.address, self.port, self.is_ssl
        )
...
class WebSocketServer:
    ...
    def port(self):
        ...
        try:
            ...
            raise RuntimeError(
                "This socket does not have a port: {!r}".format(listener)
            ) from None

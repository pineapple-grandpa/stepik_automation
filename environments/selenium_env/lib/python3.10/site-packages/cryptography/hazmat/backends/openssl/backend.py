...
class Backend:
    ...
    def _evp_pkey_to_private_key(self, evp_pkey) -> PRIVATE_KEY_TYPES:
        ...
        if key_type == self._lib.EVP_PKEY_RSA:
            ...
            self.openssl_assert(res == 1)
            return self.load_der_private_key(self._read_mem_bio(bio), password=None)
    ...
    def pkcs7_serialize_certificates(
        ...
        certs = list(certs)
        if not certs or not all(isinstance(cert, x509.Certificate) for cert in certs):
        ...
        if encoding is serialization.Encoding.PEM:
            res = self._lib.PEM_write_bio_PKCS7_stream(bio_out, p7, self._ffi.NULL, 0)
        else:

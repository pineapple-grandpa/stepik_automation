    ...
    def __init__(self, dotted_string: str) -> None:
        ...
        for node in nodes:
            try:
                ...
                raise ValueError(f"Malformed OID: {dotted_string} (non-integer nodes)")
            if node_value < 0:
                raise ValueError(
        ...
        if len(nodes) < 2:
            raise ValueError(
                f"Malformed OID: {dotted_string} " "(insufficient number of nodes)"
            )

        if intnodes[0] > 2:
            raise ValueError(
                f"Malformed OID: {dotted_string} " "(first node outside valid range)"
            )

        if intnodes[0] < 2 and intnodes[1] >= 40:
            raise ValueError(
                f"Malformed OID: {dotted_string} " "(second node outside valid range)"
            )
